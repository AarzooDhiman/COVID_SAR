...
def basic_run( inpath, outpath,auth, fmth, v):
    ...
    classes = pool.starmap(author_fam_positive, zip(user_ids, [path]*len(user_ids), [out_path]*len(user_ids), [auth]*len(user_ids), [fmth]*len(user_ids))) ## user author_fam_positive2 to create folders with author positive and family positive tweets, use author_fam_positive to use author positive and family positive tweets for the SAR calcualtion. author_fam_positive2 needs to be run only once to get the required tweets set.
    pool.close()
    print (classes)
