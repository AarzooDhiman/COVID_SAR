...
def find_best_pair(auth_df, fam_df, days): ## method that finds the best pair of author-positive and family-positive tweets within 14 days interval for a user 
    ...
    df_all = auth_df1.merge(fam_df1, how='outer') #finding a cross join of author-positive and family-positive tweets of a user
    ...
    df_all1 = df_all[(df_all['delta']>=(0-days)) & (df_all['delta']<=days) & (df_all['delta']!=0)]  #filtering the cross join to get the pairs within the delta time

    
    if df_all1.shape[0]>0: #if there exists any tweet pair with delta>=-14 or delta<=14
    ...
    elif df_all1.shape[0]==0: # if no pair with delta within the given time
        if df_all.shape[0]>0: # if there existed any tweet in author-positive or family-positive
        ...
        else: # if there existed no tweet at all to start with
            return "nothing"
...
def author_fam_positive(user, path, out_path, auth_th, fmth_th):
    ...
    if os.path.exists(f'../data/user_positives/{user}/family_positive.csv'):
        family_positive= pd.read_csv(f'../data/user_positives/{user}/family_positive.csv')
    ...
    if family_tweets.shape[0]>0:
        if family_max.shape[0] > 0:
            ...
            if family_positive.shape[0]<=75: ####removing the outlier case (just one)
                ...
                family_positive.to_csv(f'../data/user_positives/{user}/family_positive.csv')
...
def calSAR(classes0):
    ...
    si_df.to_csv('output_files/user_si_p0.csv')
...
def plot_data(tab, all_probs,  path, ath,fth,v, r):
    ...
    table.to_csv(f'output_files/SAR_scores.csv', index=None)
    return table
...
def basic_run( inpath, outpath,auth, fmth, v):
    ...
    user_ids = get_users4()   # getting user IDs in cohort
    ...
    out_path = outpath
    print (out_path)
    pool = Pool(n_p)  # running separate thread for each user to make it faster
    
    print (list(zip(user_ids, [path]*len(user_ids), [out_path]*len(user_ids), [auth]*len(user_ids), [fmth]*len(user_ids)))[:2])
    
    classes = pool.starmap(author_fam_positive, zip(user_ids, [path]*len(user_ids), [out_path]*len(user_ids), [auth]*len(user_ids), [fmth]*len(user_ids))) ## use author_fam_positive2 to create folders with author positive and family positive tweets, use author_fam_positive to use author positive and family positive tweets for the SAR calcualtion. author_fam_positive2 needs to be run only once to get the required tweets set.
    pool.close()
    print (classes)
...
def main():
    ...
    if len(args)>5:
        ...
        for run in [1]: 
            for v in [14]:
                x =-0.63 # threshold for the Author classifier
                y = 0.028 # threshold for the fam classifier
                print ("=====Thresholds======")
